...
SSMApplet.prototype = {
	...
	_update: function() {
		...
		[shortInfo, longInfo] = this.memDataProvider.data(this.memMode);
	...
	_bind_settings: function() {
		...
		this.settings.bindProperty(Settings.BindingDirection.IN,			"mem_mode",			"memMode",		null, null);
...
MemDataProvider.prototype = {
	...
	data: function(memMode = "used", longInfoTab = 2, longInfoSize = 5) {
		this._fetch_data();
		let overall = this.currentReadings[0];
		if (memMode == "active") {
			overall = overall + this.currentReadings[1] + this.currentReadings[2];
		} 
